...
public class DateTimeUtils {
	...
	* Date
	...
	public static String getIntervalTime(Date date, Date otherDate) {
		int seconds = getIntervalSeconds(date, otherDate);
		return getTime(seconds);
	}
	public static String getUseTime(Date date) {
		Date curDate = new Date();
		int seconds = getIntervalSeconds(curDate, date);
		return getTime(seconds);
	}
	/**
	 * 
	...
	* @return
	...
	/**
	* <p>Title: isCross</p> 
	* <p>Description: 判断一个时间fisrtTime是否在一个时间区间内[secondStartTime,secondEndTime](区间为闭区间)</p> 
	* @param fisrtTime
	* @param secondStartTime
	* @param secondEndTime
	* @return
	 */
	public static boolean isCross(Date fisrtTime,Date secondStartTime,Date secondEndTime){		
		if((fisrtTime.before(secondEndTime) && fisrtTime.after(secondStartTime)) || fisrtTime.compareTo(secondStartTime) == 0 ||  fisrtTime.compareTo(secondEndTime) == 0){
			return true;
		}
		return false;
	}
	 ...
	 */
	public static List<Date> getMonthDayListBeforFay(int year, int month){  
		List<Date> list=new ArrayList<Date>();
		int monthDayNum=getMonthDayNum(year, month);
		Calendar cal = Calendar.getInstance();  
		for(int i=1;i<monthDayNum;i++){
	...
	public static Date getOclock(Date date, int hour){		
		date = setTime(date, hour, 0, 0);
		return date;
	}

	public static Date getDateYmd(Date date){		
		date = setTime(date, 0, 0, 0);
		return date;
	}
	...
	public static Date combineDateTime(Date date, String time){
		Date tt = StringToDate(time, DateStyle.HH_MM_SS.value);
		date = (Date) date.clone();
		date.setHours(tt.getHours());
		date.setMinutes(tt.getMinutes());
		date.setSeconds(tt.getSeconds());
		return date;
	}
	...
	public static float getHours(long seconds){
		float fhours = (float)seconds/(60*60);
		fhours   =  (float)(Math.round(fhours*100))/100;		 
		return fhours;
	}
	public static float getHours(String date){
		long secs = getSecondsOfDay(date);
		return getHours(secs);
	}
	@SuppressWarnings("deprecation")
	public static long getSecondsOfDay(String date){
		Date d = StringToDate(date, DateStyle.HH_MM_SS.value);
		return d.getSeconds();
	}
	...
	@SuppressWarnings("deprecation")
	public static long getLeftSecondsOfDay(Date date){		
		return 24*60*60 - date.getSeconds();
	}
	
	public static String dateToYmdhms(Date date){
		return DateToString(date, DateStyle.YYYY_MM_DD_HH_MM_SS.value);
	}
	public static String dateToYmd(Date date){
		return DateToString(date, DateStyle.YYYY_MM_DD.value);
	}
	public static Date ymdhmsToDate(String date){
		return StringToDate(date, DateStyle.YYYY_MM_DD_HH_MM_SS.value);
	}
	public static Date hmsToDate(String date){
		return StringToDate(date, DateStyle.HH_MM_SS.value);
	}
	//yyyy-MM-dd
	...
	public static String getCurStrYmd(){
	...
	public static Date getTomorrowTime(Date time){		
		Date date= addDay(new Date(), 1);
		return mergeDateTime(date, time);
	}
	
	public static Date getTomorrowTime(String t){
		return getTomorrowTime(hmsToDate(t));
	}
	public static Date getTodyTime(Date time){
		Date date= new Date();
		return mergeDateTime(date, time);
	}
	
	public static Date getTodyTime(String t){
		return getTodyTime(hmsToDate(t));
	}
}
